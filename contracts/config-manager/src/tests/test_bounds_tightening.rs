...
//!   * `update_carrying_fee_config` `max_skew_rate_bps` ceiling — bounds the
//!     maximum annualized dominant-side charge.
//!   * `update_protocol_limits` `adl_pnl_bps` floor — must reject values
//!     below `MIN_ADL_PNL_BPS` (= 50%, stops admin from configuring
//!     continuous ADL).
//!   * `update_carrying_fee_config` `slope2_bps` ceiling — must reject values
...
use crate::{CarryingFeeConfig, ConfigManagerError, FeeSplits};
use shared::constants::{
    BPS, DEFAULT_BASE_BORROW_RATE_BPS, DEFAULT_MAX_SKEW_RATE_BPS, DEFAULT_OPTIMAL_UTILIZATION_BPS,
    DEFAULT_SLOPE1_BPS, MAX_COOLDOWN_DURATION, MAX_SLOPE2_BPS, MIN_ADL_PNL_BPS,
};
...
fn test_update_protocol_limits_cooldown_at_ceiling_succeeds() {
    ...
    client.update_protocol_limits(&admin, &limits);
    assert_eq!(
        client.get_protocol_limits().cooldown_duration,
...
// ---------------------------------------------------------------------------
// update_carrying_fee_config — slope2_bps ceiling
// ---------------------------------------------------------------------------
...
fn test_update_borrow_rate_slope2_at_ceiling_succeeds() {
    ...
    client.update_carrying_fee_config(&admin, &cfg);
    assert_eq!(client.get_carrying_fee_config().slope2_bps, MAX_SLOPE2_BPS);
}
